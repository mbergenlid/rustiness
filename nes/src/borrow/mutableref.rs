...
mod test {
    ...
    fn test() {
        let v = &mut 6;
        let mut_ref_box: MutableRef<dyn SomeTrait> = MutableRef::Box(Box::new(5));
        let mut_ref_borrowed: MutableRef<dyn SomeTrait> = MutableRef::Borrowed(v);
    ...
    fn test_mutable_references_in_vector() {
        let v = &mut 6;
        let mut vector: Vec<MutableRef<dyn SomeTrait>> =
