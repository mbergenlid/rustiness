...
impl Memory for PPUMemory {
    fn get(&self, address: Address, sub_cycles: u8) -> u8 {
        ...
        if address < 0x2000 {
            ...
            self.name_tables.get(address, sub_cycles)
    ...
    fn set(&mut self, address: Address, value: u8, sub_cycle: u8) {
        ...
        if address < 0x2000 {
            ...
            self.name_tables.set(address, value, sub_cycle);
        } else if address >= 0x3F00 && address < 0x3F20 {
            let address = address as usize;
            self.palettes[(address & 0xFC) >> 2][address & 0x3] = value;
        } else {
            self.basic_memory.set(address, value, sub_cycle);
        }
    }
}
...
pub mod tests {
    ...
    fn test_horizontal_read_mirroring() {
        ...
        for address in 0x2000..0x2400 {
            assert_ne!(ppu_mem.get(address, 0), ppu_mem.get(address + 0x800, 0));
        }
    ...
    fn test_entire_name_table_mirroring() {
        ...
        for address in 0x0000..0x2000 {
            assert_eq!(0, ppu_mem.get(address, 0));
        }
