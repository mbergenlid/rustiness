...
impl<'a> Sprite for &'a [u8] {
    ...
    fn flip_horizontal(&self) -> bool {
        return self[2] & 0x40 > 0;
    }
    fn flip_vertical(&self) -> bool {
        return self[2] & 0x80 > 0;
    }
    fn is_back(&self) -> bool {
        return self[2] & 0x20 != 0;
    }
}

use ppu::screen::{Rectangle, Screen};
pub struct Sprites {
    data: [u8; 64 * 4],
    address: u8,
}
...
impl Sprites {
    ...
    pub fn render_back_sprites<T>(&self, screen: &mut T)
    where
        T: Screen + Sized,
    {
    ...
    pub fn render_front_sprites<T>(&self, screen: &mut T)
    where
        T: Screen + Sized,
    {
        for sprite_index in (0..64).rev() {
